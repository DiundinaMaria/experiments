...
class SNTPServer:
    ...
    def prepare_response(self, request, receive_time):
        ...
        return SNTPpack(self.delay,
                        ...
                        receive_time=receive_time + self.delay
                        )
...
class SNTPpack:
    SNTP_MODE = {'reserved': 0, 'symmetry active': 1, 'symmetry passive': 2,
                 'client': 3, 'server': 4, 'broadcast': 5}

    def __init__(self, delay=0, stratum=3, version=4, mode=4, originate_time=0,
                 receive_time=0):
        ...
        self.receive_timestamp = receive_time  # время приема (This value is the time at which the client request arrived at the server in 64-bit time-stamp format.)
    ...
    def create_package(self):
        ...
        sntp_recive_timestamp = self.convert_time_to_sntp(self.receive_timestamp)
    ...
    def num_to_bin(self, num, length):
        """Возвращает число num в виде бинарной строки длины length"""
        return bin(num)[2:].rjust(length, '0')

    def convert_time_to_sntp(self, time):
        """Преобразовывает время в формат sntp(добавляет поправку на 70 лет)"""
        time_shift = 2208988800  # 70 лет в секундах
